...
import { Filter, ChevronDown, ChevronUp, AlertTriangle, AlertCircle, Info, Star, CheckCircle2, StickyNote } from "lucide-react";
              ...
              <div className="overflow-x-auto">
                <Table>
                  ...
                  <TableBody>
                    {filteredSubmissions.map((submission) => {
                      ...
                      return (
                        ...
                        >
                                  ...
                                  <ScrollArea className="h-[400px]">
                                    <div className="p-4 space-y-4">
                                      ...
                                      {(() => {
                                        ...
                                        if (allResolvedForSubmission.length === 0) return null;
                                        
                                        return (
                                          <div className="mt-6 pt-4 border-t space-y-2">
                                            <div className="font-semibold text-muted-foreground flex items-center gap-2 pb-2">
                                              <CheckCircle2 className="h-4 w-4 text-green-500" />
                                              Resolved ({allResolvedForSubmission.length})
                                            </div>
                          ...
                          <TableCell>{isAbsent ? <span className="text-muted-foreground">—</span> : submission.touched_base}</TableCell>
                          <TableCell className="text-destructive">{isAbsent ? <span className="text-muted-foreground">—</span> : submission.calls_not_taken}</TableCell>
                          <TableCell>{isAbsent ? <span className="text-muted-foreground">—</span> : submission.others}</TableCell>
                          <TableCell>
                            {isAbsent ? (
                              <span className="text-muted-foreground">—</span>
                            ) : (
                          ...
                          <TableCell>{isAbsent ? <span className="text-muted-foreground">—</span> : submission.sm_rp_to_enrolled}</TableCell>
                          <TableCell>
                            {isAbsent ? (
                              <span className="text-muted-foreground">—</span>
                            ) : (
                          ...
                          <TableCell>{isAbsent ? <span className="text-muted-foreground">—</span> : submission.fu_rp_to_enrolled}</TableCell>
                          <TableCell>
                            {isAbsent ? (
                              <span className="text-muted-foreground">—</span>
                            ) : (
                    ...
                    {filteredSubmissions.length > 0 && (() => {
                      // Calculate totals - exclude absent/absent-only rows
                      const presentSubmissions = filteredSubmissions.filter(s => 
                      ...
                      return (
                        <TableRow className="bg-muted/50 font-bold border-t-2 border-primary hover:bg-muted/50">
                          <TableCell colSpan={10} className="text-base">TOTAL</TableCell>
                          <TableCell>
