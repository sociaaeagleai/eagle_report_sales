...
export const ExportMenu = ({ data, getUserName }: ExportMenuProps) => {
  ...
  const exportToCSV = () => {
    const headers = [
      "Date", "Employee", "Attendance Status", "Absence Type", "Source",
      ...
      "Attendance Notes", "Admin Notes"
    ];

    const rows = data.map(s => {
      const isAbsent = s.isAbsentOnly || s.attendance?.status === 'absent';
      const anomalies = isAbsent ? { hasAnomalies: false, critical: [], warnings: [], info: [], success: [] } : detectAnomalies({
      ...
      return [
        ...
        (s as any).attendance?.absence_type || "N/A",
        isAbsent ? '—' : s.source.join(" | "),
        isAbsent ? '—' : s.calls_dialled,
        isAbsent ? '—' : s.calls_taken,
        isAbsent ? '—' : s.rapport_built,
        isAbsent ? '—' : s.touched_base,
        isAbsent ? '—' : s.calls_not_taken,
        isAbsent ? '—' : s.others,
        isAbsent ? '—' : s.disqualified,
        isAbsent ? '—' : calculatePercentage(s.calls_taken, s.calls_dialled),
        isAbsent ? '—' : calculatePercentage(s.calls_not_taken, s.calls_dialled),
        isAbsent ? '—' : s.sm_rp,
        isAbsent ? '—' : s.sm_enrolled,
        isAbsent ? '—' : s.sm_rp_to_enrolled,
        isAbsent ? '—' : calculatePercentage(s.sm_rp + s.sm_enrolled, s.calls_taken),
        isAbsent ? '—' : s.fu_rp,
        isAbsent ? '—' : s.fu_enrolled,
        isAbsent ? '—' : s.fu_rp_to_enrolled,
        isAbsent ? '—' : calculatePercentage(s.fu_rp + s.fu_enrolled, s.calls_taken),
        ...
        (s as any).attendance?.notes || "",
        (s as any).admin_notes || ""
      ];
    });
  ...
  const exportToExcel = () => {
    ...
    const rows = data.map(s => {
      ...
      return [
        ...
        isAbsent ? '—' : s.others,
        isAbsent ? '—' : s.disqualified,
        isAbsent ? '—' : Number(calculatePercentage(s.calls_taken, s.calls_dialled)),
        isAbsent ? '—' : Number(calculatePercentage(s.calls_not_taken, s.calls_dialled)),
        isAbsent ? '—' : s.sm_rp,
        isAbsent ? '—' : s.sm_enrolled,
        isAbsent ? '—' : s.sm_rp_to_enrolled,
        isAbsent ? '—' : Number(calculatePercentage(s.sm_rp + s.sm_enrolled, s.calls_taken)),
        isAbsent ? '—' : s.fu_rp,
        isAbsent ? '—' : s.fu_enrolled,
        isAbsent ? '—' : s.fu_rp_to_enrolled,
        isAbsent ? '—' : Number(calculatePercentage(s.fu_rp + s.fu_enrolled, s.calls_taken))
      ];
    });
    ...
    // Summary Sheet
    const totalCalls = data.filter(s => !s.isAbsentOnly && s.attendance?.status !== 'absent').reduce((sum, s) => sum + s.calls_dialled, 0);
    const totalTaken = data.filter(s => !s.isAbsentOnly && s.attendance?.status !== 'absent').reduce((sum, s) => sum + s.calls_taken, 0);
    const totalAbsent = data.filter(s => s.isAbsentOnly || s.attendance?.status === 'absent').length;
    ...
    const summaryData = [
      ...
      ["Total Present", data.length - totalAbsent],
