...
export const AdvancedFilters = ({ filters, onFilterChange, profiles, sources, subSources }: AdvancedFiltersProps) => {
  ...
  const toggleSubSource = (subSource: string) => {
    const newSubSources = filters.selectedSubSources.includes(subSource)
      ? filters.selectedSubSources.filter(s => s !== subSource)
      : [...filters.selectedSubSources, subSource];
    updateFilter('selectedSubSources', newSubSources);
  };

  const toggleAbsenceType = (type: string) => {
    const newTypes = filters.absenceType.includes(type)
      ? filters.absenceType.filter(t => t !== type)
      : [...filters.absenceType, type];
    updateFilter('absenceType', newTypes);
  };
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          ...
          <div className="space-y-2">
            <Label>Sub-Source {filters.selectedSubSources.length > 0 && `(${filters.selectedSubSources.length})`}</Label>
            <Popover>
              <PopoverTrigger asChild>
                <Button variant="outline" className="w-full justify-between">
                  {filters.selectedSubSources.length === 0 ? "Select sub-sources..." : `${filters.selectedSubSources.length} selected`}
                  <ChevronsUpDown className="ml-2 h-4 w-4 shrink-0 opacity-50" />
                </Button>
              </PopoverTrigger>
              <PopoverContent className="w-[300px] p-0" align="start">
                <Command>
                  <CommandEmpty>No sub-sources found.</CommandEmpty>
                  <CommandGroup className="max-h-64 overflow-auto">
                    {subSources.map((subSource) => (
                      <CommandItem
                        key={subSource}
                        onSelect={() => toggleSubSource(subSource)}
                      >
                        <div className={cn(
                          "mr-2 flex h-4 w-4 items-center justify-center rounded-sm border border-primary",
                          filters.selectedSubSources.includes(subSource)
                            ? "bg-primary text-primary-foreground"
                            : "opacity-50 [&_svg]:invisible"
                        )}>
                          <Check className="h-4 w-4" />
                        </div>
                        <span className="capitalize">{subSource}</span>
                      </CommandItem>
                    ))}
                  </CommandGroup>
                </Command>
            ...
            {filters.selectedSubSources.length > 0 && (
              <Button 
                variant="ghost" 
                size="sm" 
                onClick={() => updateFilter('selectedSubSources', [])}
                className="h-7 text-xs"
              >
                Clear
              </Button>
        ...
        <div className="flex gap-2 flex-wrap mt-2">
          ...
          <Button 
            ...
            onClick={() => {
              ...
              updateFilter('startDate', yesterdayStr);
              updateFilter('endDate', yesterdayStr);
            }}
          >
            Yesterday
          </Button>
          <Button 
            ...
            onClick={() => {
              ...
              updateFilter('startDate', firstDayOfWeek.toISOString().split('T')[0]);
              updateFilter('endDate', lastDayOfWeek.toISOString().split('T')[0]);
            }}
          >
            This Week
          </Button>
          <Button 
            ...
            onClick={() => {
              ...
              updateFilter('startDate', firstDayOfMonth.toISOString().split('T')[0]);
              updateFilter('endDate', lastDayOfMonth.toISOString().split('T')[0]);
            }}
          >
            This Month
          </Button>
      ...
      {/* Notes & Attendance Filters */}
      <div className="space-y-4">
        <h4 className="font-medium text-sm text-muted-foreground">Notes & Attendance</h4>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="space-y-2">
            <Label>Attendance Status</Label>
            <Select value={filters.attendanceStatus} onValueChange={(value) => updateFilter('attendanceStatus', value)}>
              <SelectTrigger>
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
          ...
          <div className="space-y-2">
            <Label>Absence Type {filters.absenceType.length > 0 && `(${filters.absenceType.length})`}</Label>
            <Popover>
              <PopoverTrigger asChild>
                <Button variant="outline" className="w-full justify-between">
                  {filters.absenceType.length === 0 ? "Select types..." : `${filters.absenceType.length} selected`}
                  <ChevronsUpDown className="ml-2 h-4 w-4 shrink-0 opacity-50" />
                </Button>
              </PopoverTrigger>
              <PopoverContent className="w-[250px] p-0" align="start">
                <Command>
                  <CommandGroup>
                    {['sick_leave', 'casual_leave', 'emergency', 'unapproved'].map((type) => (
                      <CommandItem
                        key={type}
                        onSelect={() => toggleAbsenceType(type)}
                      >
                        <div className={cn(
                          "mr-2 flex h-4 w-4 items-center justify-center rounded-sm border border-primary",
                          filters.absenceType.includes(type)
                            ? "bg-primary text-primary-foreground"
                            : "opacity-50 [&_svg]:invisible"
                        )}>
                          <Check className="h-4 w-4" />
            ...
            {filters.absenceType.length > 0 && (
              <Button 
                variant="ghost" 
                size="sm" 
                onClick={() => updateFilter('absenceType', [])}
                className="h-7 text-xs"
              >
                Clear
              </Button>
          ...
          <div className="space-y-2">
            <Label>Admin Notes Search</Label>
            <Input 
              type="text" 
              placeholder="Search admin notes..."
              value={filters.adminNotesSearch}
              onChange={(e) => updateFilter('adminNotesSearch', e.target.value)}
            />
          </div>

          <div className="space-y-2">
            <Label>Has Admin Notes</Label>
            <Select value={filters.hasAdminNotes} onValueChange={(value) => updateFilter('hasAdminNotes', value)}>
              <SelectTrigger>
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
          ...
          <div className="space-y-2">
            <Label>Attendance Notes Search</Label>
            <Input 
              type="text" 
              placeholder="Search attendance notes..."
              value={filters.attendanceNotesSearch}
              onChange={(e) => updateFilter('attendanceNotesSearch', e.target.value)}
            />
          </div>

          <div className="space-y-2">
            <Label>Has Attendance Notes</Label>
            <Select value={filters.hasAttendanceNotes} onValueChange={(value) => updateFilter('hasAttendanceNotes', value)}>
              <SelectTrigger>
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
