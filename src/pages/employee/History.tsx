...
const History = () => {
  ...
  useEffect(() => {
    checkAuthAndFetch();
  }, [currentPage, pageSize, startDate, endDate, selectedSources, selectedSubSources]);
  ...
  // Apply client-side filters for source/subsource (date filtering is done server-side)
  ...
  }, [submissions, selectedSources, selectedSubSources]);
  ...
  const checkAuthAndFetch = async () => {
    ...
    try {
      ...
      // Fetch attendance data for the current page dates
      ...
      // Ensure source is properly typed as string[]
      const allRecords = [...submissionsWithAttendance, ...absentOnlyDays]
        .map(record => ({
          ...record,
