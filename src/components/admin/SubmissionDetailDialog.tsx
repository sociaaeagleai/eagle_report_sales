...
import { Phone, TrendingUp, Target, Calendar, Star, CheckCircle2, StickyNote } from "lucide-react";
...
interface SubmissionDetailDialogProps {
  ...
  onNoteSave?: (submissionId: string, note: string) => void;
}
...
export const SubmissionDetailDialog = ({
  ...
  // Update adminNote when submission changes
  useEffect(() => {
    if (submission) {
      // For absent-only rows, use attendance.notes
      const noteValue = submission.isAbsentOnly 
        ? (submission.attendance?.notes || "")
        : (submission.admin_notes || "");
      setAdminNote(noteValue);
    }
  }, [submission]);
  ...
  const handleSaveNote = async () => {
    ...
    try {
      if (submission.isAbsentOnly) {
        ...
        // Normal submission - save to daily_submissions
        const { error } = await supabase
      ...
      toast({
        title: "Note saved",
        description: "Admin note has been updated successfully.",
      });

      if (onNoteSave) {
        onNoteSave(submission.id, adminNote);
      }
    } catch (error) {
        ...
        <div className="space-y-6">
          ...
          <div className="space-y-3 p-4 bg-muted/50 rounded-lg border">
            ...
            <Textarea
              ...
              placeholder="Add notes about this submission (e.g., employee mindset, weather, lead quality, context)..."
              className="min-h-[120px] resize-none"
            />
            <p className="text-xs text-muted-foreground">
              Use this space to remember important details about the day, employee context, external factors, or any other observations.
            </p>
          </div>
        </div>
      </DialogContent>
